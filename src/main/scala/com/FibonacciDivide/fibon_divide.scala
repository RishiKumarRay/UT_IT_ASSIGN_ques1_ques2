...
class fibon_divide extends Divide with fibonacci {
  ...
  override def fibonacci(NumberOfTerms: Int): ListBuffer[Int] = {
    ...
    if (NumberOfTerms < 0) throw new NegativeArraySizeException("Negative number is not Allowed in Fibonacci Series")
